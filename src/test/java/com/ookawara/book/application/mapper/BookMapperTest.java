...
class BookMapperTest {
    ...
    void 指定した値が完全一致した本のデータを返す() {
        List<Book> book = bookMapper.findBook("鬼滅の刃・1", LocalDate.of(2016, 6, 8), false, 1);
    ...
    @DataSet("datasets/books.yml")
    @Transactional
    void 指定したカテゴリーIDが完全一致した本のデータを返す() {
        List<Book> book = bookMapper.findBook("", null, null, 1);
        assertThat(book)
                .contains(new Book(2, "鬼滅の刃・1", LocalDate.of(2016, 6, 8), false, 1));
    }
    ...
    @DataSet("datasets/books.yml")
    @Transactional
    void 指定した書籍名と発売日と購入状況が全て完全一致した本のデータを返す() {
        List<Book> book = bookMapper.findBook("鬼滅の刃・1", LocalDate.of(2016, 6, 8), false, 0);
        assertThat(book)
                .contains(new Book(2, "鬼滅の刃・1", LocalDate.of(2016, 6, 8), false, 1));
    }
    ...
    @DataSet("datasets/books.yml")
    @Transactional
    void 指定した書籍名と発売日が全て完全一致した本のデータを返す() {
        List<Book> book = bookMapper.findBook("鬼滅の刃・1", LocalDate.of(2016, 6, 8), null, 0);
        assertThat(book).contains(new Book(2, "鬼滅の刃・1", LocalDate.of(2016, 6, 8), false, 1));
    }
    ...
    @DataSet("datasets/books.yml")
    @Transactional
    void 指定した値が全て完全一致しないときsql文の条件により空のデータを返す() {
        List<Book> book = bookMapper.findBook(" ", LocalDate.of(2000, 1, 1), null, 0);
        assertThat(book).isEmpty();
    }
